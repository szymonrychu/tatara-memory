...
# release.yml passes the authoritative tag in via VERSION; git describe must
# not be trusted to pick it, because a re-run of a release that failed after
# cutting its tag leaves two semver tags on one commit and describe picks the
# lower one. git describe stays only as the ci.yml fallback (pre-tag builds).
