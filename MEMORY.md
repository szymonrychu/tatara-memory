...
- 2026-06-12 Orphaned-item recovery (issue #4): a worker crash between `ClaimNextItem` (status->`running`) and `MarkItemDone` left items stuck `running`; `ClaimNextItem` claims only `pending`, so `Resume` re-ran the job but never reclaimed the orphan and the job drained to a wrong count (silent data loss). Fix: `JobStore.RequeueOrphanedItems` resets `running`->`pending` for items in unfinished (`queued`/`running`) jobs only (terminal jobs untouched); `Pool.Resume` calls it before listing/scheduling. Idempotent retry relies on `CreateMemory` keying on the item idempotency key. Narrowly scoped to `running` orphans; the separate non-atomic Done/Failed counter race remains tracked in ROADMAP.
