...
- 2026-06-12 Ingest per-item timeout (szymonrychu/tatara-memory#6): `processItem` ran `CreateMemory` on the long-lived startup ctx, so a hung LightRAG call blocked a worker and `Stop()` could not interrupt the in-flight item (the lightrag HTTP client's 30s timeout is only a fallback). Fix: `Pool.SetItemTimeout` wraps each item's ctx (covering CreateMemory + source indexing) in `context.WithTimeout`; configurable via `INGEST_ITEM_TIMEOUT`/`--ingest-item-timeout`/chart `ingestItemTimeout`, default 5m, 0 disables. Chose a setter over a new constructor arg to avoid churning the public `NewPool*` signatures and their tests.
