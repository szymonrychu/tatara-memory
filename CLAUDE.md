...
- Every change declares significance. Agents set it via the required
...
- Agents NEVER merge PRs. The pipeline merges bot-authored PRs on green
  required checks, cuts the semver tag from the label, publishes
  artifacts at vX.Y.Z, propagates the version pin to the parent repo,
  and tatara-helmfile auto-applies it to the cluster. The operator
  closes the originating issue on apply success.
