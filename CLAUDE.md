# CLAUDE.md - tatara-memory

## What this repo is

The platform's memory service: an OIDC-gated REST API over LightRAG plus the
code-graph store that `tatara-memory-repo-ingester` fills. Every agent pod's
recall and code-graph MCP tools terminate here.

## What this repo is NOT

- Not the ingester. Walking a repo and producing the graph is
  `tatara-memory-repo-ingester`; this repo stores and serves it.
- Not the MCP surface. Agents reach it through `tatara-cli`, never directly.
...
- Rule 16 exists because of this repo: the v0.1.1 -> v0.1.2 upgrade was
  hand-patched with `kubectl set image`, and the orphaned field-managers broke
  helm server-side apply on the next sync.
- A schema or embedding-model change is a reindex, not a rolling update. Say so
  in `MEMORY.md` and treat it as `major`.
